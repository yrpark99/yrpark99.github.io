...
* USB 저장 장치 마운트: [WSL 2에 Linux 디스크 탑재](https://learn.microsoft.com/ko-kr/windows/wsl/wsl2-mount-disk) 참조
  ...
  * 아래와 같이 언마운트시킬 수 있다. (예로 **/mnt/e** 경로를 언마운트)
    ...
    $ sudo umount /mnt/e
    ```
* USB 장치(device) 연결하기: [USB 디바이스 연결](https://learn.microsoft.com/ko-kr/windows/wsl/connect-usb) 참조  
  노트: WSL2 배포판의 Linux 커널 버전이 5.10.60.1 이상이어야 하므로, 사전에 확인하여 이것보다 오래된 버전이면 `wsl --update` 명령을 실행하여 업데이트한다.
  1. [usbipd-win](https://github.com/dorssel/usbipd-win) 툴을 설치한다. 예를 들어 winget으로 아래와 같이 설치할 수 있다.
     ```sh
     C:\>winget install usbipd
     ```
     또는 최신 버전을 설치하려면 [usbipd-win 릴리즈](https://github.com/dorssel/usbipd-win/releases) 페이지에서 직접 다운받아서 설치한다.
  1. 현재 USB device 목록 및 상태 얻기
     ```sh
     C:\>usbipd list
     ```
     예로 USB-to-Serial converter인 경우에 아래 캡쳐 예와 같이 나온다. (STATE가 **Not shared**로 나옴)
     ```sh
     BUSID  VID:PID    DEVICE                                                        STATE
     10-3   0403:6015  USB Serial Converter                                          Not shared
     ```
  1. WSL2에서 사용할 USB device를 아래와 같이 bind 시킨다. (관리자 권한의 콘솔에서 실행)
     ```sh
     C:\>usbipd bind --busid {BUSID}
     ```
     테스트 예로 아래와 같이 USB-to-Serial converter를 bind 시킨다.
     ```sh
     C:\>usbipd bind --busid 10-3
     ```
     다시 목록을 확인해 보면 아래 캡쳐와 같이 STATE가 **Shared**로 바뀌어 있다.
     ```sh
     C:\>usbipd list
     BUSID  VID:PID    DEVICE                                                        STATE
     10-3   0403:6015  USB Serial Converter                                          Shared
     ```
  1. 이제 WSL2 배포판이 실행된 상태에서, 아래와 같이 attach 시키면 bind 된 USB device를 WSL에 연결되어 WSL2에서 사용할 수 있게 된다.
     ```sh
     C:\>usbipd attach --wsl --busid {BUSID}
     ```
     테스트로 아래와 같이 attach 시킨다.
     ```sh
     C:\>usbipd attach --wsl --busid 10-3
     ```
     이제 다시 목록을 확인하면 아래와 같이 STATE가 **Attached**로 바뀌어져 있는 것을 볼 수 있다.
     ```batch
     C:\>usbipd list
     BUSID  VID:PID    DEVICE                                                        STATE
     10-3   0403:6015  USB Serial Converter                                          Attached
     ```
     또, WSL2 배포판에서 `dmesg` 명령으로 확인해 보면 아래 발췌 예와 같이 해당 USB device 들이 인식되었음을 볼 수 있다.
     ```sh
     usb 1-1: New USB device found, idVendor=0403, idProduct=6015, bcdDevice=10.00
     usb 1-1: New USB device strings: Mfr=1, Product=2, SerialNumber=3
     usb 1-1: Product: FT231X USB UART
     usb 1-1: Manufacturer: FTDI
     usb 1-1: SerialNumber: D30BPZYZ
     ...
     ```
     `lsusb` 명령으로 확인해 보면 아래 예와 같이 나온다.
     ```sh
     ...
     ```
     결과로 `/dev/ttyUSB0` device node가 생성되었음을 확인할 수 있고, 이 장치를 사용하면 된다.
  1. Bind 된 USB device는 Windows 콘솔에서 아래와 같이 detach 시킬 수 있다.
     ```sh
