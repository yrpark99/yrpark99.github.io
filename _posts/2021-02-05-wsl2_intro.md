...
1. 아래와 같이 빌드한다.
   ...
   $ make -j
   ```
   결과로 `vmlinux`와 `arch/x86/boot/bzImage` 파일이 생성된다. 이 중에 아무 파일이나 Windows 임시 디렉토리로 복사한 후, 아래와 같이 WSL을 종료시킨다.
   ```sh
   C:\>wsl --shutdown
   ````
1. `C:\Windows\System32\lxss\tools\` 디렉토리에 있는 `kernel` 파일을 다른 이름으로 변경한 후, `vmlinux`나 `bzImage` 파일 중에 원하는 파일을 복사하여 `kernel` 이름으로 변경한다.
