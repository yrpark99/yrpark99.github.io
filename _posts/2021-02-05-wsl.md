...
참고로 [WSL Github 페이지](https://github.com/microsoft/WSL/)에서 WSL 관련한 좀더 자세한 내용을 얻을 수 있다.  
<br>
...
## WSL 실행
WSL 설치가 끝나면 WSL 아이콘이 생성되고 이것을 실행하면 터미널이 뜨고 WSL에 로그인된다. 그런데 이 터미널은 구린 환경이므로 나는 이것 대신에 주로 `Windows Terminal`이나 `MobaXterm`을 사용한다. 이것들은 자동으로 WSL을 인식하고 그외 많은 기능을 제공한다.  
또는 Windows 콘솔에서 아래와 같이 커맨드로도 접속할 수 있다.
```sh
C:\>wsl ~ -d <배포판 이름>
```
그런데 이 경우 현재 경로가 /mnt/c 경로로 되고, home 디렉토리로 이동하려면 `cd` 명령을 입력해야 하므로, 약간 불편하다. 그래서 나는 아래와 같이 실행하는데, 이렇게 하면 home 디렉토리로 바로 이동한다.
```sh
C:\>wsl ~ -d <배포판 이름>
```
물론 사실상 커맨드에서 이렇게 직접 실행하기 보다는 `Windows Terminal`에서 해당 profile의 "commandline"에 이렇게 세팅해서 사용한다.  
<br>
...
## WSL 배포판 삭제
WSL 배포판 삭제는 일반적인 앱 제거와 마찬가지로 Windows 설정 메뉴의 `앱` -> `앱 및 기능`에서 (즉, `프로그램 추가/제거`) 하면 된다. 또는 아래와 같이 커맨드로도 할 수 있다.
```bash
...
* PATH에서 Windows 경로 제거하기  
  ...
  alias code='/mnt/c/Program\ Files/Microsoft\ VS\ Code/bin/code.exe'
  ```
* 네트워크 드라이브로 연결하기  
...
* 32bit 지원하기  
  ...
  ```
* GUI 프로그램 실행하기  
  * X server를 이용하면 되는데, Windows에서 사용 가능한 X server에는 `VcXSrv`, `Xming` 또는 Microsoft store에 있는 `GWSL` 등이 있다. (참고로 `MobaXterm` 프로그램은 WSL을 지원하고 자체적으로 X server를 내장하고 있음)
...
* NFS 서버 설치  
  ...
  1. WSL2의 네트워크는 Hyper-V 가상화 기반으로 동작하므로, `Hyper-V 관리자`를 관리자 권한으로 실행시킨 후, "가상 스위치 관리자" 항목을 클릭한다.
