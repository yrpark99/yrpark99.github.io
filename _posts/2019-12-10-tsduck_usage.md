...
1. TS 파일의 TS 패킷 덤프
   ...
   ```
   참고로 ECM 데이터를 볼 때와 같이 중복된 것은 출력되지 않게 하려면 아래와 같이 `--no-duplicate` 옵션을 추가하면 된다.
   ```sh
   $ tstables --pid <ECM_PID_값> --raw-dump --no-duplicate <TS 파일>
   ```
...
1. 입력 파일을 UDP로 multicast 전송하기 예 (아래 예에서는 multicast IP 주소는 **239.0.0.11**, 포트 번호는 **9999** 사용, TTL은 **10** 지정)
   ...
   > * **--infinite**: 파일 무한 반복
   > * **-P regulate**: 정상 속도(1배속)로 play
   > * **-P zap {service_id}**: 서비스 ID에 해당하는 프로그램만 play

   이것을 VLC media player에서 재생하려면 메뉴에서 `미디어` -> `네트워크 스트림 열기` -> `네트워크` 탭에서 네트워크 주소에 `"udp://@239.0.0.11:9999"`와 같이 입력한 후에 재생 버튼을 누르면 된다.  
   <br>
   또는 Linux에서 아래 예와 같이 `ffplay` 툴로 play 시킬 수 있다.
   ```sh
