...
## WSL 배포판 실행 유지
그런데 현재 WSL 설계상 systemd를 활성화시킨 상태에서 배포판의 로그인 shell을 종료하면 WSL이 자동으로 약 15초 후에 해당 배포판을 종료시키는 것 같다. 배포판의 로그인 shell을 종료시키더라도 배포판이 stop 되는 것을 막기 위해서는 여러 방법들이 있겠지만, 구글링을 통해서 내가 찾은 방법은 다음과 같다.
1. 배포판에서 아래와 같이 `keychain` 패키지를 설치한다.
   ```sh
   $ sudo apt install keychain
   ```
2. 이후 배포판에서 `/etc/profile.d/keep_wsl_running.sh` 파일을 아래와 같이 작성한다. (파일 이름은 상관없음)
   ```sh
   #!/usr/bin/env sh
   eval $(keychain -q)
   ```
