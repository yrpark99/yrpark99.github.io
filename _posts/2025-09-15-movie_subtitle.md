...
## Claude Code로 번역 테스트
위 테스트 이후 몇 개월이 지난 후에 Claude Code 계정이 생겨서, 이번에는 동일한 SRT 파일을 Claude Code로 번역을 해 보았다. 번역시 문맥을 파악하여 일관되게 번역할 수 있도록 SRT 파일 전체를 입력으로 넣고, 간단히 주인공에 대한 정보를 주면서 직역보다는 의역을 하라고 지시하였더니, 결과로 상당히 고품질의 한글 번역 파일을 얻을 수 있었다. (사실상 사람이 별도록 수정할 필요가 없을 정도로)  
이제는 발전된 AI 기술 덕분에 다른 언어의 자막도 아주 손쉽게 한글로 번역할 수 있는 시대가 되었다. 👍
