...
## 맺음말
위와 같이 기존 자동화 툴을 Jira 이슈까지 업데이트하도록 구현하여, 단순히 실행 파일만 실행시키면 key 입수/조작/릴리즈의 전 과정이 자동으로 처리되도록 하였다. 역시 단순 반복 작업은 자동화 툴을 구현하여 사용하는 것이 휴먼 에러도 없고 시간도 뺏기지 않고, 속도도 더 빨라서 좋은 점이 많다.  
추후에도 Jira 관련하여 자동화가 필요한 경우에 참조하기 위하여 간략히 정리해 보았다.
