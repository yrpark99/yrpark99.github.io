...
1. GitLab에서 빈 저장소를 생성한다. 예로 https://localhost/gitlab/test/manifest 저장소를 생성한다.
1. 생성한 저장소를 아래와 같이 clone 받은 후, 해당 디렉토리로 이동한다.
   ```shell
   $ git clone https://localhost/gitlab/test/manifest.git
   $ cd manifest/
   ```
1. 해당 디렉토리에서 Manefest 파일의 디폴트 이름인 **default.xml** 파일을 아래 예와 같이 작성한다.
   ```xml
...
repo 저장소를 구성해 보기 위해서 구글링을 했더니, repo의 사용법은 많이 나와 있었지만 정작 저장소 구성 관련은 찾기가 힘들었다.  직접 repo 저장소를 구성해 보고 테스트 하면서 방법을 알고 나니 간단하였다. 😋
