...
Swagger UI 테스트를 위해서는 Go 소스에서 아래와 같이 import를 추가하고, 라우터를 설정해 주면 된다. (아래에서 import의 `{module_name}` 부분은 본인의 모듈 이름으로 대체해야 함)
```go
import (
...
가장 기본적이고 범용적인 방법이다. 아래 예와 같이 테스트할 수 있다.
```sh
$ curl -X 'POST' localhost/api -H 'accept: text/plain' -H 'Content-Type: multipart/form-data' -F 'id=xxx' -F 'pw=xxx'
```

* <span><font color=blue>httpie</font></span> 이용  
curl과 비슷하지만 조금 더 편리할 수 있다. 아래와 같이 httpie 패키지를 설치해서 테스트할 수 있다.
```sh
$ sudo apt install httpie
$ http -v POST localhost/api
```
...
테스트 할 수 있는 익스텐션에는 `REST Client`, `Thunder Client` 등과 같은 익스텐션을 사용할 수 있다.
...
또 `Makefile`을 아래와 같이 작성하여 빌드시에 `-tags` 옵션을 주어 swagDisable.go 또는 swagEnable.go 둘 중 하나의 파일만 빌드되도록 하였다.
```makefile
ifeq ($(SWAGGER), y)
TAGS = -tags swagger
endif
