...
마찬가지로 `Display` 대신에 아래와 같이 `Send`를 이용하면 메일 팝업을 띄우지 않고 바로 메일을 전송하게 된다.
```go
...
func main() {
    ...
    for i := 1; i <= int(count); i++ {
        ...
        fmt.Println("=== MAIL", i, " ===")
        fmt.Println("sender:", sender.Value())
        fmt.Println("to:", to.Value())
        fmt.Println("subject:", subject.Value())
