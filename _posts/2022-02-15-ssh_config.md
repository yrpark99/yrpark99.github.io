...
참고로 SSH 클라이언트로 Windows를 사용하는 경우에도 Linux의 경우와 별반 다르지 않다.
...
대부분의 경우에 디렉토리인 경우에는 `-rp`, 파일인 경우에는 `-p` 옵션이 주로 사용되는데, 여기에서 **-r** 옵션은 하위의 디렉터리 및 파일까지 recursive 복사, **-p** 옵션은 속성(원본 파일의 변경 시간, 접근 시간, 퍼미션 등) 보존을 의미한다.  
