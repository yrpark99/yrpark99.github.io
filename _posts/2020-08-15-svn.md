...
SVN 서버 구성시 사용한 유용한 팁을 일부 정리해 본다.
...
## SVN 동작시 메일 보내기
1. SVN에서 특정 동작시에 (예: lock, unlock, commit 등) 메일을 보내려면 해당 hook 파일에서 메일을 발송하는 프로그램을 호출하면 되는데, 메일 발송 프로그램은 Python, Node.js, Go 등 본인이 원하는 언어로 작성하면 된다.  
...
1. 아래와 같이 아파치 서버를 재시작한다.
   ...
   ```
1. 이제 웹브라우저에서 [http://server_address/svn/repo_name/](http://server_address/svn/repo_name/) 주소 예와 같이 접속하면 웹브라우저에서 바로 SVN 파일들의 내용을 확인할 수 있다.
