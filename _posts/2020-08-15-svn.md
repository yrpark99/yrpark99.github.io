...
svn_log_externals() {
    local path="$1"
    echo -e "\e[91m[SVN logs for $path]\e[0m"
    svn log "$path"
    local externals_paths=$(svn propget svn:externals "$path" 2> /dev/null)
    if [ -n "$externals_paths" ]; then
        IFS=$'\n'
        for external_path in $externals_paths; do
            external_path=$(echo "$external_path" | awk '{print $1}')
            new_path="$path/$external_path"
            echo
            svn_log_externals "$new_path"
        done
    fi
}
...
if [ $# -eq 0 ]; then
    ...
    svn_log_externals $(echo "$1" | sed 's:/$::')
fi
```
