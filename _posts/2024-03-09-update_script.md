...
## 스크립트 작성 이유
나는 개발 서버로 Ubuntu를 사용 중인데, 대부분의 유용한 tool 들이 우분투의 APT 패키지로는 업데이트가 자주 되지 않는다.  
그래서 최신 버전으로 업데이트하려면 각 tool 들의 GitHub 페이지에서 최신 릴리즈를 직접 다운로드 받아서 설치해야 하는 번거로움이 있었다.  
그런데 [https://api.github.com/repos](https://api.github.com/repos) 페이지를 통해서 해당 툴의 최신 버전 정보를 얻을 수 있다는 사실을 알게 되어서, 이를 이용한 업데이트 스크립트를 작성하였고, 사용해 보니 꽤 편리하여 공유한다.  
<br>
